...
def split_audio_vad(audio_path, audio_name, target_dir, split_seconds=10.0):
    ...
    target_folder = os.path.join(target_dir, audio_name.split("_")[0])
    # wavs_folder = os.path.join(target_folder, 'wavs')
    os.makedirs(target_folder, exist_ok=True)
    start_time = 0.0
    count = 0
    ...
    for i in range(num_splits):
        ...
        if i == num_splits - 1:
            end_time = audio_dur
        output_file = f"{target_folder}/{audio_name}_seg{count}.wav"
...
def split_audio_vad1(audio_path, audio_name, target_dir, split_seconds=10.0):
    SAMPLE_RATE = 16000
    audio_vad = get_audio_tensor(audio_path)
    segments = get_vad_segments(
        audio_vad,
    ...
    for i in range(num_splits):
        ...
        if i == num_splits - 1:
            end_time = audio_dur
        output_file = f"{target_folder}/{audio_name}_seg{count}.wav"
        audio_segs.append(audio_active[int(start_time * 1000) : int(end_time * 1000)])
        # audio_seg.export(output_file, format="wav")
        start_time = end_time
        count += 1
    return audio_segs
